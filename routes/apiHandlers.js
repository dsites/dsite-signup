const { hash } = require('dpayjs/lib/auth/ecc');
...
async function handleRequestSms(req) {
    ...
    try {
        await services.sendSMS(
            phoneNumber,
            `${phoneCode} is your dPay confirmation code`
        );
    } catch (cause) {
...
/**
 * Create the account on the blockchain using dpayjs
...
async function handleCreateAccount(req) {
    ...
    try {
        ...
        // dpayjs error messages are so long that the log is clipped causing
