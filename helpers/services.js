...
const rpcNode = getEnv('DPAYJS_URL');
if (rpcNode) {
    dpay.api.setOptions({ url: rpcNode });
}
    ...
    if (DEBUG_MODE) {
        ...
        switch (method) {
            case 'is_email_registered':
                return (params.email || params[0]) === 'taken@dsite.io';
        ...
        return dpay.api.signedCallAsync(
            `conveyor.${method}`,
            params,
            conveyorAccount,
            conveyorKey
...
async function createAccount(payload) {
    if (DEBUG_MODE) {
      ...
      return dpay.broadcast.sendAsync([op1, op2], createAccountWif);
    }
}
...
async function checkUsername(username) {
    ...
    const [account] = await dpay.api.getAccountsAsync([username]);
    return !!account;
}
